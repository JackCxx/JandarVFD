	...
	<div style="height: 100%;">
		<a-layout class="container">
			...
			<a-layout-sider
				...
				<flow-attr :plumb="plumb" :flowData="flowData" :flowOptions="flowOptions" :select.sync="currentSelect"></flow-attr>
			</a-layout-sider>
		</a-layout>
		<a-modal
...
export default {
  ...
  mounted () {
    const that = this
    // let config = JSON.parse(JSON.stringify(this.nodeExtraConfig))
    let config = {}
    if (this.options != null && this.options != '') {
      config = JSON.parse(JSON.stringify(this.options))
    }
    that.dealCompatibility()
    that.initNodeSelectArea()
    that.initJsPlumb()
    that.listenShortcut()
    if ($.isEmptyObject(config.loadJson)) {
      that.initFlow()
    } else {
      that.initFlowForJSON(config.loadJson)
    }
    if (config.assessPersons !== 'undefined' && config.assessPersons != null) {
      that.flowOptions.assessPersons = config.assessPersons
    } else {
      that.flowOptions.assessPersons = []
    }
    if (config.personTypes !== 'undefined' && config.personTypes != null) {
      that.flowOptions.personTypes = config.personTypes
    }
    if (config.areaTree !== 'undefined' && config.areaTree != null) {
      that.flowOptions.areaTree = config.areaTree
    } else {
      that.flowOptions.areaTree = []
    }
    if (config.ccPersons !== 'undefined' && config.ccPersons != null) {
	  that.flowOptions.ccPersons = config.ccPersons
    } else {
  ...
  methods: {
    ...
    initFlow () {
      ...
      if (that.flowData.status == flowConfig.flowStatus.CREATE) {
        ...
        if (that.options != null && that.options != '') {
          if (this.options.flowTitle !== 'undefined' && this.options.flowTitle != null) {
			  that.flowData.attr.name = this.options.flowTitle
          } else {
			  that.flowData.attr.name = 'flow-' + ZFSN.getId()
          }
          if (this.options.flowId !== 'undefined' && this.options.flowId != null) {
			  that.flowData.attr.id = this.options.flowId
          } else {
			  that.flowData.attr.id = 'flow-' + ZFSN.getId()
          }
          if (this.options.flowTablename !== 'undefined' && this.options.flowTablename != null) {
			  that.flowData.attr.talbename = this.options.flowTablename
          } else {
			  that.flowData.attr.talbename = ''
          }
        }
      } else {
        that.loadFlow()
    ...
    saveFlow () {
	  ...
	  if (this.options != null && this.options != '') {
		  config = JSON.parse(
		  		  JSON.stringify(this.options, function (key, val) {
